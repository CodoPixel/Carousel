...
interface JSCarouselOptions {
    autonav?: boolean;
    autonav_delay?: number;
    tabs?: boolean;
    navigations_buttons?: boolean;
    // just in case the user wants several carousels,
    // without that, the window will have duplicated events.
...
class JSCarousel {
    ...
    protected readonly allow_autonav: boolean;
    protected readonly allow_navbuttons: boolean;
    protected readonly autonav_delay: number;
    protected readonly disable_keyboard_navigation: boolean;
    protected readonly tabs: NodeListOf<HTMLElement> | null;
    ...
    public constructor(container: HTMLElement, options?: JSCarouselOptions) {
        ...
        if (windows) {
            for (let i = 0; i < windows.length; i++) {
                (windows[i] as HTMLElement).style.left = (this.width * i) + "px";
            }
        } else {
            throw new Error("There is no window in the carousel.");
        }
        ...
        if (this.allow_navbuttons)this._enable_navigation_buttons();
        if (this.allow_tabs && this.tabs) this._enable_tabs_buttons();
        if (!this.disable_keyboard_navigation) this._enable_keyboard_navigation();
     ...
     * @private
     */
    private _enable_navigation_buttons(): void {
     ...
     * @private
     */
    private _enable_tabs_buttons(): void {
        if (this.allow_tabs && this.tabs) {
     ...
     * @private
     */
    private _enable_keyboard_navigation(): void {
        window.addEventListener("keydown", (e: KeyboardEvent) => {
            ...
            if (cltr_up_in_window) {
                if (this.tabs) this.tabs[this.current_index].focus();
            }
            ...
            if (focus_on_tabs) {
                ...
                if (is_left || is_right) {
                    (this.tabs as NodeListOf<HTMLElement>)[this.current_index].focus();
                    e.preventDefault();
                }
            }
        });
     ...
     * @protected
     */
    protected have_tabs_focus(): boolean {
        if (this.allow_tabs && this.tabs) {
     ...
     * @protected
     */
    protected have_windows_focus(): boolean {
        const windows = this.container.querySelectorAll(".carousel-content");
        if (windows) {
    ...
    public activate_panel(index: number): void {
        ...
        if (this.tabs) {
            ...
            if (tab) {
                ...
                if (pos) {
                    if (!pos.classList.contains("pos-active")) pos.classList.add("pos-active");
     ...
     * @protected
     */
    protected hide_window(window_content: Element): void {
        window_content.setAttribute("aria-hidden", "true");
        const focusHandler = new FocusableElement();
        const focusable_elements = focusHandler.getKeyboardFocusableElements(window_content as HTMLElement);
     ...
     * @protected
     */
    protected show_window(window_content: Element): void {
     ...
     * @protected
     */
    protected move(index: number): void {
        ...
        if (distance !== 0) {
            for (let window_content of Array.from(all_windows)) {
                const left_property = (window_content as HTMLElement).style.left;
                const current_left_pos = parseFloat(left_property);

                (window_content as HTMLElement).style.left = (current_left_pos + distance) + "px";
